...
public class BluetoothService {
	...
	public boolean setPin(Class btClass, BluetoothDevice btDevice,String str) throws Exception {
        ...
        setPinMethod.invoke(btDevice,new Object[]{str.getBytes()});
        return true;
    }
	...
	public int connectBluetoothSocket() {
		try {
			...
			Log.i(TAG,"建立socket失败，" + e.toString());
			return -1;
		}
		...
		public void run() {
            ...
            while(true){  
                try {  
                    ...
                    //把读取到的数据发送给UI进行处理  
                    String message = new String(buff,"gb2312");
                    Message msg = handler.obtainMessage(BluetoothService.MESSAGE_READ,  
                            len, -1, message);  
                    msg.sendToTarget();  
                } catch (IOException e) {  
                	Log.e(TAG, "蓝牙socket监听失败" + e.toString());
                }  
            }  
	...
	public void sendData(String message) throws Exception {
		if (outputStream != null) {
			byte[] buff = message.getBytes("gb2312");  
	        outputStream.write(buff);
	        Log.i(TAG,"蓝牙发送数据成功");
		} else {
			Log.i(TAG,"发送失败，输出流为空");
		}
    }  
}
